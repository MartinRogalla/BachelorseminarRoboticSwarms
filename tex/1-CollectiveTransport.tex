...
This problem is a composite problem, composed\todo{SB: consisting?} of the already mentioned problems formation and source localization. 
...
Because the swarm robots do not calculate the exact distance between them, this algorithm can be defined as a range-free algorithm.\todo{SB: is it? they do communicate over limited range?}
...
Because the framework calculates the exact range between each robot in the swarm, this algorithm is range-based.\todo{SB: what do you mean by framework? and isn't it range-free / centralized, because it is controlled by a user?}
...
The properties are summarized in the Table~\ref{table_alg_coltrans1}.
...
An algorithm that only uses very simple robots is an algorithm that is based on granular convection, also known as the Brazil Nut Effect. 
The algorithms can be implemented in three different ways: (1) A swarm with homogeneous robots with no explicit communication, 
...
As the robots are randomly placed around the payload and the goal location, the payload gets kicked around by the vibrating swarm robots, until the payload is kicked to the target location.
...
A short summarization of these algorithms is given in Table~\ref{table_alg_coltrans2}.
