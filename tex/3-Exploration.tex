...
When comparing the \emph{performance} of the algorithms we mean the explored area per distance travelled. When comparing the \emph{scalability} of the algorithms we mean the scalability of the number of robots.
	...
	Its goal is to gain as much information as possible when travelling towards a new location.
	...
	The cells have, dependent to their occupancy probability, a certain state which is either open, unkown or occupied.
	Each open cell that is adjacent to an unkown cell is labeled as a frontier cell.
	...
	It then broadcasts a bid acording to: the target of the robots in the sensor range, the distance to the targeted cell and a nearness measure.
	This nearness measure is a factor which is dependent of the number of robots in the neighbourhood to keep the robots together and thus sustain communication.
	...
	The algorithm then assigns all robots to a certain region by distance calculation, making a difference between acessible and inacessible regions.
	After that each robot gets assigned to a frontier cell in its region.
	Robots with acessible regions will be assigned to frontier cells by the distance between the robot and the frontier cell and a penalty when the cell is close to another frontier cell assigned to a different robot.
	Robots with inacessible regions get another penalty for the distance from the frontier cell and the centroid of their own region.
