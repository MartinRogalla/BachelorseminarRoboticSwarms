...
\subsection{Discussion}
	...
	This creates a situation where we want the swarm robots to stay together to share information efficiently. 
	But, in contrast, we also want the robots to spread widely to gain as much new information as possible to gain as much new information as possible. 
	The problem is combining these two goals and create a balanced solution. \\
	In the market economy based algorithm this problem is addressed by optimizing the utility of and the distance traveled towards each cell. 
	However, this implementation is centralized and therefore less scalable. 
	In the bidding and frontier based algorithm not only the utility and the distance to be traveled towards it are taken into account, but also a certain nearness measure.
	This nearness measure is also used in the foraging algorithms, using virtual pheromones and multi-hop communication to take this into account.
	This measure makes the robots tend to stay together, much like what has been discussed in the formation problem.  
	Every distributed robotic swarms exploration algorithm should thus incorporate a solution to the formation problem in some way. 
	If this technique gets fully optimized, the robots use each others knowledge as much as possible and at the same explore in an efficient way.\\
